...
float PerformanceCounters::GetVideoThreadUsage()
{
  return s_state.video_thread_usage;
}

float PerformanceCounters::GetVideoThreadAverageTime()
{
  return s_state.video_thread_time;
}
...
void PerformanceCounters::Update(GPUBackend* gpu, u32 frame_number, u32 internal_frame_number)
{
  ...
  s_state.last_video_thread_time = video_thread_time;

  s_state.core_thread_usage = static_cast<float>(static_cast<double>(core_thread_delta) * pct_divider);
  s_state.core_thread_time = static_cast<float>(static_cast<double>(core_thread_delta) * time_divider);
  s_state.video_thread_usage = static_cast<float>(static_cast<double>(video_thread_delta) * pct_divider);
  s_state.video_thread_time = static_cast<float>(static_cast<double>(video_thread_delta) * time_divider);
  ...
  VERBOSE_LOG("FPS: {:.2f} VPS: {:.2f} CPU: {:.2f} RNDR: {:.2f} GPU: {:.2f} Avg: {:.2f}ms Min: {:.2f}ms Max: {:.2f}ms",
              s_state.fps, s_state.vps, s_state.core_thread_usage, s_state.video_thread_usage, s_state.gpu_usage,
